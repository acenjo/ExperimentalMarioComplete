...
function draw() {
	...
	if(gamestate === "PLAY"){
	   ...
	   console.log(player.y)
		if(keyDown("space")  && (player.y > 782) ||(touches.length > 0)){
		   player.velocityY = -20
		   touches = []
		}
		...
		if(player.isTouching(shootingstarGroup)){
			...
			lives = lives + 1
			punchSound.play()
			//I couldn't find a good sound effect in SoundBible for "Level Up" or "Ding" so I have to stick with the punch one.
...
function spawnSpikes(){
	if(frameCount % 80 === 0){
		mariospike = createSprite(width + 30, height - 60, 50, 100)
		mariospike.addImage(mariospikeImage)
		mariospike.scale = 0.1
...
function spawnBirds(){
	if(frameCount % 150 === 0){
		...
		bird.y = Math.round(random(500, 600))
		bird.scale = 0.5
...
function spawnStars(){
	if(frameCount % 400 === 0){
		shootingstar = createSprite(width + 20, height - 300, 20, 20)
		shootingstar.addImage(shootingstarImage)
		shootingstar.y = Math.round(random(500, 650))
